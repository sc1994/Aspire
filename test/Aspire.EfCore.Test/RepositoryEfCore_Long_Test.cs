...
{
    public class RepositoryEfCore_Long_Test : RepositoryEfCore_Test<EfCoreEntity_Long_Test, long>
    {
        [Fact]
        ...
        public override async void QuerySqlRawAsync_Test()
        {
            ...
            var r = await Repository.QuerySqlRawAsync("SELECT * FROM EfCoreEntityLongTest WHERE Id IN ({0},{1},{2})", ids.Select(x => (object)x).ToArray());
        ...
        {
            var ids = await Repository.AddRangeThenIdsAsync(CreateEntity(), CreateEntity(), CreateEntity());
            ids = new List<long> { 999 }.Concat(ids).ToArray();
            var r = await Repository.ExecuteSqlRawAsync("UPDATE EfCoreEntityLongTest SET Age = Age + {0} WHERE Id IN ({1},{2},{3})", ids.Select(x => (object)x).ToArray());
            Assert.Equal(3, r);
        }
    }
}
